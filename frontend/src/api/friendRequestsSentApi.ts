...
// API function to fetch sent friend requests
export const friendRequestsSentApi = async () => { 
  ...
  try {
    const response = await fetch(`${API_URL}friendRequests/sent/pending/${userId}`, {
      method: "GET",
      headers: {
    ...
    if (!response.ok) {
      ...
      throw new Error(data.message || "Failed to fetch sent friend requests");
    }

    return data;
  } catch (error) {
    console.error("🔥 [API] Error fetching sent friend requests:", error);
    throw error;
  }
};
